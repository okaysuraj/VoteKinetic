...
export const adminApi = {
  ...
  blockVoter: (userId: string, reason: string) =>
    request<{ message: string }>(`/admin/users/${userId}/block`, null, {
      method: 'POST',
...
export const tallyApi = {
  /** Tally votes for a closed election */
  tally: (user: User, electionId: string) =>
    request('/tally', user, { method: 'POST', body: JSON.stringify({ electionId }) }),
};
