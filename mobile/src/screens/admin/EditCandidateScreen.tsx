...
export const EditCandidateScreen = () => {
  ...
  useEffect(() => {
    if (election && candidateId) {
      const candidate = election.candidates?.find((c: any) => c.id === candidateId);
      if (candidate) {
        setName(candidate.name || '');
        setBio(candidate.bio || '');
        // party is not in schema, so we mock it
        setParty('Independent Coalition');
      }
    }
  ...
  const handleSave = async () => {
    ...
    if (!user || !electionId || !candidateId) return;

    setLoading(true);
    try {
      await electionApi.updateCandidate(user, electionId, candidateId, {
        name: name.trim(),
        bio: bio.trim(),
      });
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            ...
            setLoading(true);
            try {
              await electionApi.deleteCandidate(user, electionId, candidateId);
              navigation.goBack();
            } catch (error: any) {
  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      <View className="flex-row items-center justify-between px-4 py-4 border-b border-outline-variant bg-surface">
        ...
        <Text className="font-bold text-xl text-primary flex-1 ml-4">Edit Candidate</Text>
        <TouchableOpacity onPress={handleDelete} className="p-2 -mr-2" disabled={loading}>
          ...
          >
            {loading ? (
              ...
              <>
                ...
                <Text className="text-on-primary font-bold text-lg ml-2">Save Changes</Text>
              </>
            )}
          </TouchableOpacity>
        </ScrollView>
